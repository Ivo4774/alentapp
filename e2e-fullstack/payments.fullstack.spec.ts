...
const test = base.extend<{ memberSetup: string }>({
  memberSetup: async ({ page }, use) => {
    const uniqueMemberName = `Socio Pagos ${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    ...
    await use(uniqueMemberName);
  },
});

test.describe('Payments Full-Stack E2E - Alta', () => {

  test('debe emitir un comprobante de pago pendiente', async ({ page, memberSetup }) => {
...
test.describe('Payments Full-Stack E2E - Actualización', () => {
  ...
  test('debe registrar el cobro efectivo de un pago existente', async ({ page, memberSetup }) => {
...
test.describe('Payments Full-Stack E2E - Cancelación', () => {
  ...
  test('debe anular un pago pendiente correctamente', async ({ page, memberSetup }) => {
