...
export class CreateMedicalCertificateUseCase {
  ...
  async execute(data: CreateMedicalCertificateRequest): Promise<MedicalCertificateDTO> {
    // 1. Validar que la fecha de vencimiento sea posterior a la de emisión
    if (new Date(data.expiry_date) <= new Date(data.issue_date)) {
      throw new Error("La fecha de vencimiento es inválida");
    }

    // 2. TDD-0007: Buscar certificados previos del socio para invalidarlos
    ...
    // 3. Crear el nuevo certificado (por defecto is_validated será true según el esquema)
